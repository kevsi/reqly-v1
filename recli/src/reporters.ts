...
const SENSITIVE_PATTERN = /(token|secret|password|auth|apikey|api_key|session|jwt|bearer|ssn|credit|card)/i
    ...
    if (result.assertions && result.assertions.length > 0) {
      for (const a of result.assertions) {
        ...
        if (a.passed) {
          ...
          console.log(`${aIcon} ${a.name}  ${chalk.red(`(expected: ${redactSensitive(a.expected)}, got: ${redactSensitive(a.actual)})`)}`)
        }
      }
    }
