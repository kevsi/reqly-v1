...
function parseSimpleConfig(content: string, separator: string): RecliConfig {
  ...
  for (const line of content.split("\n")) {
    ...
    switch (key) {
      case "env": config.env = String(value); break
      case "timeout": config.timeout = Number(value); break
      case "parallel": config.parallel = value === true || value === "true"; break
      case "delay": config.delay = Number(value); break
      case "iterations": config.iterations = Number(value); break
      case "reporter": config.reporter = String(value); break
      case "output": config.output = String(value); break
      case "data": config.data = String(value); break
      case "snapshot": config.snapshot = value === true || value === "true"; break
      case "dotenv": config.dotenv = String(value); break
    }
  }
  return config
}
