...
function resolveToken(token, ctx) {
    ...
    if (lower === "uuid" || lower === "guid")
        return String(generateValue("uuid", rng));
    if (lower === "nowiso" || lower === "now.iso")
    ...
    if (floatMatch) {
        ...
        const dec = parseInt(floatMatch[3] ?? "2", 10);
    ...
    if (fakerMatch) {
        ...
        if (canonical)
            return String(generateValue(canonical, rng));
        const inferred = inferFormat(fmt);
        if (inferred)
            return String(generateValue(inferred, rng));
    }
    return undefined;
}
...
export function resolveTemplate(template, ctx) {
    return template.replace(/\{\{\s*([^{}]+?)\s*\}\}/g, (raw, token) => {
