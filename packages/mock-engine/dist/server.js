...
function adminOriginAllowed(origin, extra) {
    try {
        const u = new URL(origin);
        const host = u.hostname.toLowerCase();
        if (host === "localhost" || host === "127.0.0.1" || host === "[::1]" || host === "::1")
            return true;
        if (ADMIN_DEFAULT_ORIGINS.includes(origin))
            return true;
...
function preview(value, max = 2048) {
    return value.length > max ? value.slice(0, max) + "…" : value;
}
export function createMockServer(initialConfig, options = {}) {
    ...
    async function handle(req, res) {
        ...
        if (pathname.startsWith("/mock/__admin")) {
            ...
            // the token alone.
            const originAllowed = origin === null || adminOriginAllowed(origin, options.adminAllowedOrigins);
        ...
        const finish = (status, body, matchedRouteId, note, extraHeaders) => {
            const entry = {
                id: randomId(),
                at: started,
                method,
        ...
        if (route.latency) {
            const ms = randBetween(route.latency.minMs, Math.max(route.latency.minMs, route.latency.maxMs));
            if (ms > 0)
                await sleep(ms);
        }
        ...
        if (route.failure && rng() < route.failure.probability) {
            ...
            if (kind === "status") {
                finish(route.failure.statusCode ?? 500, JSON.stringify({ error: "simulated_failure" }), route.id, `failure:${kind}`);
                return;
            }
            if (kind === "timeout") {
                const entry = {
            ...
            if (kind === "reset")
                ...
                finish(200, '{"ok":', route.id, `failure:${kind}`); // malformed JSON
            return;
        }
        const response = selectResponse(route, { query, headers, body: parsedBody, rawBody });
        if (!response) {
            finish(500, JSON.stringify({ error: "misconfigured_route", routeId: route.id }), route.id, "no_response");
            return;
        }
        const ctx = {
            method,
        ...
        if (stateful) {
            ...
            switch (method) {
                ...
                case "DELETE": {
                    ...
                    if (!ok)
                        finish(404, JSON.stringify({ error: "not_found" }), route.id);
                    else
                        finish(200, JSON.stringify({ deleted: true, id: idFromPath }), route.id);
                    return;
                }
            }
        ...
        const looksJson = built.body.trimStart().startsWith("{") || built.body.trimStart().startsWith("[");
