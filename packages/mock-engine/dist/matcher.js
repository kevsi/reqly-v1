...
export function matchPath(pattern, actualPath) {
    ...
    if (!compiled.hasSplat && parts.length !== compiled.segments.length)
        return null;
    if (compiled.hasSplat && parts.length < compiled.segments.length - 1)
        return null;
    const params = {};
    for (let i = 0; i < compiled.segments.length; i++) {
        ...
        if (seg !== decodeURIComponent(parts[i] ?? ""))
            return null;
    }
