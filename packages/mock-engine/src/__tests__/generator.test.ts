...
describe("generate", () => {
  it("generates format-compliant strings", () => {
    const email = generate({ type: "string", format: "email" }, Math.random) as string;
    expect(email).toMatch(/@example\.com$/);
    const uuid = generate({ type: "string", format: "uuid" }, Math.random) as string;
    expect(uuid).toMatch(/^[0-9a-f-]{36}$/);
  });

  it("infers from key name when schema is loose", () => {
    const v = generate({ type: "string" }, Math.random, "email") as string;
    expect(v).toContain("@");
  });
  ...
  it("honors numeric bounds", () => {
    const n = generate({ type: "number", min: 5, max: 10 }, Math.random) as number;
...
describe("resolveTemplate", () => {
  it("injects request values", () => {
    const out = resolveTemplate(
      '{"userId":"{{request.path.id}}","page":"{{request.query.page}}","token":"{{request.header.x-token}}","who":"{{request.body.user.name}}"}',
      ctx,
    );
