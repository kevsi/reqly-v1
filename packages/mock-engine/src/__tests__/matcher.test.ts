...
import type { MockRoute } from "../matcher.js";
...
describe("evaluateRule", () => {
  ...
  it("query equals / exists / missing", () => {
    expect(evaluateRule({ target: "query", name: "page", op: "equals", value: "2" }, ctx)).toBe(
      true,
    );
  ...
  it("header equals is case-insensitive on name and case-sensitive on value", () => {
    expect(
      evaluateRule({ target: "header", name: "X-Token", op: "equals", value: "abc123" }, ctx),
    ).toBe(true);
    expect(
      evaluateRule({ target: "header", name: "X-Token", op: "equals", value: "ABC" }, ctx),
    ).toBe(false);
  });

  it("body dot-path equals / exists / regex", () => {
    expect(
      evaluateRule({ target: "body", name: "user.role", op: "equals", value: "admin" }, ctx),
    ).toBe(true);
    expect(evaluateRule({ target: "body", name: "user.email", op: "exists" }, ctx)).toBe(false);
    expect(
      evaluateRule({ target: "body", name: "user.role", op: "regex", value: "^ad" }, ctx),
    ).toBe(true);
  });

  it("contains works on strings and arrays", () => {
    expect(
      evaluateRule({ target: "body", name: "note", op: "contains", value: "world" }, ctx),
    ).toBe(true);
    expect(
      evaluateRule({ target: "body", name: "user.tags", op: "contains", value: "beta" }, ctx),
    ).toBe(true);
  });
});
