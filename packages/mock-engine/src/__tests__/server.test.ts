...
// index re-exports server internals; import types via relative for clarity
import type { MockConfig } from "../index.js";

const handles: MockServerHandle[] = [];

async function start(config: MockConfig): Promise<{ base: string; handle: MockServerHandle }> {
  const handle = createMockServer(config);
  await new Promise<void>((resolve) => {
    (handle.server as Server).listen(0, "127.0.0.1", resolve);
  });
  handles.push(handle);
...
describe("mock server (real HTTP)", () => {
  ...
  it("generates realistic schema-compliant data", async () => {
    const { base } = await start(baseConfig);
    for (let i = 0; i < 5; i++) {
      const body = (await (await fetch(`${base}/api/users`)).json()) as Record<string, unknown>;
  ...
  it("selects conditional responses by body rule and applies transforms", async () => {
    const { base } = await start({
      ...baseConfig,
      routes: baseConfig.routes.map((r) =>
        r.id === "pay"
  ...
  it("injects probabilistic failures", async () => {
    ...
    let failures = 0;
    for (let i = 0; i < 20; i++) {
      if ((await fetch(`${base}/flaky`)).status === 500) failures++;
    }
  ...
  it("returns 501 with available endpoints for unmocked paths", async () => {
    ...
    const body = (await res.json()) as { error: string; available: Array<{ path: string }> };
  ...
  it("runs stateful CRUD flows in-memory", async () => {
    ...
    const item = (await created.json()) as { id: string; label: string; score: number };
    ...
    expect(((await got.json()) as { id: string }).id).toBe(item.id);
    ...
    const updatedBody = (await updated.json()) as { label: string; id: string };
  ...
  it("serves a generated preview when a stateful list is still empty", async () => {
    const { base } = await start({
      version: 1,
      routes: [
        {
          ...
          responses: [
            {
              ...
              schema: {
                type: "object",
                properties: {
                  title: { type: "string", format: "slug" },
                  views: { type: "integer", min: 0, max: 500 },
                },
                required: ["title", "views"],
              },
            },
          ],
          stateful: { enabled: true, resource: "posts" },
        },
      ],
    });
  ...
  it("strips basePath before matching and echoes CORS when enabled", async () => {
    ...
    expect(res.status).toBe(200);
    expect(((await res.json()) as { hello: string }).hello).toBe("from-base");
