...
export function inferFormat(fieldName: string): BodySchema["format"] | undefined {
  for (const [re, fmt] of NAME_HINTS) if (re.test(fieldName)) return fmt;
  return undefined;
}

export function generateValue(
  format: NonNullable<BodySchema["format"]>,
  rng: Rng,
): string | number {
  switch (format) {
    case "email":
 ...
 */
export function generate(schema: BodySchema | undefined, rng: Rng, keyHint?: string): unknown {
  ...
  if (type === "array") {
    ...
    return Array.from({ length: count }, () =>
      generate(schema.items ?? { type: "string" }, rng, keyHint),
    );
  }
