...
function runInChild(code: string, input: TransformInput): Promise<unknown> {
  return new Promise((resolve, reject) => {
    ...
    child.on("message", (message: ChildReply) => {
      ...
      else reject(new Error(message?.error ?? "Mock transform script failed"));
    });
...
export async function runTransform(code: string, input: TransformInput): Promise<unknown> {
