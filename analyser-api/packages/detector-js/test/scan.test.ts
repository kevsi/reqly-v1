...
import { detectorJs } from "../src/index.ts";
...
const fixtures = ["express", "fastify", "nestjs", "nextjs"];

for (const name of fixtures) {
  test(`detector-js: ${name}`, async () => {
    const rootPath = path.join(import.meta.dirname, "..", "fixtures", name);
    const result = await analyze({ rootPath, detectors: [detectorJs] });
