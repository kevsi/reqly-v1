...
async function analyzeStatic(folderPath: string): Promise<SavedProject> {
  ...
  for (const f of files) {
    ...
    if (/\bjsonwebtoken\b|\bjwt\.verify\b|\bjwt\.sign\b/.test(c))
      projectAuthIndicators.hasJsonWebToken = true;
    if (
      /request\.cookies|NextResponse\.cookie|response\.cookies|cookies\.get\(|cookies\.set\(/.test(
        c,
      )
    )
      projectAuthIndicators.hasCookieAccess = true;
    if (/\.session\b|req\.session|session\[|express-session/.test(c))
      projectAuthIndicators.hasSession = true;
    if (/next-auth|NextAuth|getServerSession|getSession|useSession/.test(c))
      projectAuthIndicators.hasNextAuth = true;
    if (/@clerk\/|useAuth\(\)|currentUser\(\)|auth\(\)/.test(c))
      projectAuthIndicators.hasClerk = true;
    if (/supabase.*auth|createClient.*supabase/.test(c))
      projectAuthIndicators.hasSupabaseAuth = true;
    if (/firebase\/auth|signInWith|onAuthStateChanged/.test(c))
      projectAuthIndicators.hasFirebaseAuth = true;
  }

  const language = detectLanguage(filteredFiles);
  const framework = detectFramework(filteredFiles);
  const relevantFiles = filteredFiles.filter((f) => isRelevantFile(f.path, framework));
  ...
  for (const r of routes) {
    ...
    if (!r.authRequired && fp) {
      ...
      if (
        projectAuthIndicators.hasNextAuth &&
        /getServerSession|getSession|auth\(\)/.test(content)
      ) {
        r.authRequired = true;
        r.authType = "cookie";
        r.reasonings?.push("NextAuth getServerSession dans le handler");
      }
      if (projectAuthIndicators.hasClerk && /currentUser\(\)|auth\(\)|clerkClient/.test(content)) {
        r.authRequired = true;
        r.authType = "middleware";
        r.reasonings?.push("Clerk auth dans le handler");
      }
      if (
        projectAuthIndicators.hasSupabaseAuth &&
        /supabase\.auth\.getUser|supabase\.auth\.getSession/.test(content)
      ) {
        r.authRequired = true;
        r.authType = "cookie";
        r.reasonings?.push("Supabase auth dans le handler");
      }
    }
  }
    ...
    if (!r.authRequired) {
      ...
      if (inference.required) {
        ...
        const allowed = [
          ...
          "cookie",
          "passport",
        ] as DetectedRoute["authType"][];
        r.authType =
          inference.type && allowed.includes(inference.type) ? inference.type : "middleware";
    ...
    if (!r.authRequired) {
      try {
        ...
        if (mm) {
          ...
          if (
            /passport\.authenticate|ensureauth|requireauth|verifyjwt|verifytoken|authenticatejwt|authguard|checkauth|isauth\b|auth\s*\(|login_required|permission_required|userrequired|adminonly|rolesrequired/.test(
              rawLower,
            ) ||
...
function parseLocalDefinitions(content: string): Set<string> {
  ...
  for (const m of content.matchAll(/(?:export\s+)?function\s+([A-Za-z_$][\w$]*)\s*\(/g))
    defs.add(m[1]);
  for (const m of content.matchAll(
    /(?:export\s+)?(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:async\s*)?(?:\(|[A-Za-z_$][\w$]*)/g,
  ))
    defs.add(m[1]);
  return defs;
}
...
function isImportAuth(
  ...
  return /(?:passport|jsonwebtoken|jwt|next-auth|nextauth|auth|oauth|token|jose|argon2|bcrypt|crypto|clerk|supabase|firebase|auth0|okta|cognito|oidc)/i.test(
    info.source,
  );
}
