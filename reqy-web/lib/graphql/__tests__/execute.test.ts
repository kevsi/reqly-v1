...
function makeProxyResponse(opts: { status: number; body: string; durationMs?: number }): Response {
  return new Response(
    JSON.stringify({
      status: opts.status,
...
describe("executeGraphQL", () => {
  ...
  it("includes proxy auth headers", async () => {
    document.cookie = `proxy_visitor=${"a".repeat(48)}; path=/`;
    const fetchMock = vi
      .mocked(fetch)
      .mockResolvedValueOnce(makeProxyResponse({ status: 200, body: "{}" }));
    ...
    document.cookie = "proxy_visitor=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  });
  ...
  it("handles HTTP errors (proxy returns non-2xx)", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      new Response(JSON.stringify({ error: "Bad Gateway", status: 502, headers: {} }), {
        status: 502,
      }),
    );
