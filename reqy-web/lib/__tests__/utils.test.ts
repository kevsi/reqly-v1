...
describe("extractUnresolvedPlaceholders", () => {
  it("returns an empty list when nothing is unresolved", () => {
    expect(
      extractUnresolvedPlaceholders([
        { field: "url", text: "https://api.example.com/users" },
        { field: "body", text: '{"a": 1}' },
        { field: "authToken", text: "" },
      ]),
    ).toEqual([]);
  });

  it("reports the variable name and its field", () => {
    expect(extractUnresolvedPlaceholders([{ field: "url", text: "https://{{host}}/x" }])).toEqual([
      { field: "url", name: "host" },
    ]);
  });

  it("scans headers alongside url/body/authToken", () => {
    const result = extractUnresolvedPlaceholders([
      { field: "url", text: "https://{{missingUrl}}/x" },
      { field: "body", text: "" },
      { field: "header:Authorization", text: "Bearer {{missingToken}}" },
      { field: "header:X-Key", text: "{{apiKey}}" },
    ]);
    expect(result).toEqual([
      { field: "url", name: "missingUrl" },
      { field: "header:Authorization", name: "missingToken" },
      { field: "header:X-Key", name: "apiKey" },
    ]);
  });

  it("dedupes repeated placeholders within a field but keeps other fields", () => {
    const result = extractUnresolvedPlaceholders([
      { field: "url", text: "{{token}}/{{token}}" },
      { field: "body", text: '{"t": "{{token}}"}' },
    ]);
    expect(result).toEqual([
      { field: "url", name: "token" },
      { field: "body", name: "token" },
    ]);
  });

  it("ignores null/undefined texts and handles spaces in braces", () => {
    expect(
      extractUnresolvedPlaceholders([
        { field: "url", text: null },
        { field: "body", text: undefined },
        { field: "authToken", text: "{{ spaced }}" },
      ]),
    ).toEqual([{ field: "authToken", name: "spaced" }]);
  });
});

describe("replaceLocalhostPort", () => {
  it("replaces port in localhost URL", () => {
    expect(replaceLocalhostPort("http://localhost:3000/api/test", 8080)).toBe(
