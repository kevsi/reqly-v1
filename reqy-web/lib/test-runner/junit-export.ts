...
export function toJUnitXml(report: CollectionRunReport): string {
  ...
  // Counts are derived from the actual results (not the caller-supplied
  // summary) so the XML always matches the testcase list below it — e.g.
  // after a partial "re-run failed" merge where a stale summary could
  // under-report the real total.
  const total = report.results.length;
  const failed = report.results.filter((r) => r.status === "fail").length;
  const errored = report.results.filter((r) => r.status === "errored").length;
