...
const buildRecentRequests = (history: HistoryItem[], limit?: number) => {
...
export default function DashboardPage() {
  ...
  const topSlowEndpoints = useMemo(() => buildTopSlowEndpoints(filteredHistory), [filteredHistory]);
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isEmpty
                ...
                : `Monitoring ${filteredHistory.length} request${filteredHistory.length !== 1 ? "s" : ""} across ${new Set(filteredHistory.map((h) => h.endpoint || h.url)).size} endpoint${new Set(filteredHistory.map((h) => h.endpoint || h.url)).size !== 1 ? "s" : ""}`}
            </p>
          </div>
          {!isEmpty && (
          ...
          {allRequests.length > REQ_PAGE_SIZE && (
            <div className="flex items-center justify-between border-t px-6 py-3 shrink-0">
              <span className="text-xs text-muted-foreground">
                Page {reqPage + 1} of {Math.ceil(allRequests.length / REQ_PAGE_SIZE)}
              </span>
